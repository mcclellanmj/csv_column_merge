...
parser.add_argument('--ignore-empty', action='store_true', help='Will not join the particular cell if it is completly empty')
...
def merged_generator(source, *to_merge):
	for row in source:
		...
		to_yield = [column for (i, column) in enumerate(row) if not (i in to_merge[1:])]
		yield to_yield
